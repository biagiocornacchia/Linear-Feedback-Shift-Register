...
seed_list = [0b1100101011000110 , 0b1100101011000110]
...
def check_correctnes(start_index, period):
    ...
    with open('LFSR_OUTPUT_STREAM_PYTHON.txt', "r") as output_python:
        with open('modelsim/LFSR_OUTPUT_STREAM.txt', "r") as file_modelsim:
            ...
            file_modelsim.seek(start_index) # Mult 2 is needed because files contain also the "\n"
            data_modelsim = file_modelsim.read(65535*2).splitlines()

            output_python.seek(start_index)
            data_python = output_python.read(65535*2).splitlines()
            
            for i in range(0, period):
                #if(i < 100 or i > 65530):
                #    print(str(i) + ": " + data_modelsim[i] + " - " + data_python[i])
...
if (set_seed == 'Y'):
    create_testbench()
    print("[] Add the new file 'tb/LFSR_GEN_BY_PHYTON_tb.vhd' into your project on ModelSim, compile it and run the simulation. After that you can continue the test.")
